...
listZones();
// Function to toggle ad visibility
function toggleAd(show) {
  const adDiv = document.getElementById('ad');
  if (adDiv) {
      adDiv.style.display = show ? 'flex' : 'none';
  }
}

// Hide ads when a game is opened
function openZone(file) {
  // Hide the ad when opening a zone
  toggleAd(false);
...
// Show ads when game is closed
function closeZone() {
  zoneViewer.style.display = "none";
  zoneViewer.removeChild(zoneFrame);
  // Show the ad again when returning to the main page
  toggleAd(true);
}